...
\subsection{Design of distributed implementation}
% intro parallelisation, distribution

This section describes how we create and evaluate a distributed version of the C++
algorithm. We used multicore CPUs with straightforward
intra-node thread parallelisation, not the GPU implementations described
in the previous section. The justification is twofold:
...
Secondly, usage of GPUs in the distributed setting would necessitate staging of
the relevant parts of $\pi/\phi$ from the host to GPU memory in each iteration,
which would consume most of the performance gain offered by the GPU because
the algorithm is so data-intensive.
% The details of the multi-threaded parallelism
% will be discussed only where appropriate.
...
\subsubsection{RDMA Distributed Key-Value Store}
...
constant throughout the execution of the algorithm: no records
are dynamically inserted or deleted from the DKV store after their initial insertion,
although the value for any key can be updated. Additionally, all
keys are of the same type and represent a unique vertex in the graph
so, they can be represented as integer identifiers. Each key
has a vector of $K+1$ floats as its value.
%
...
single stage either reads values or updates values, and the algorithm ensures that updates
...
\subsubsection{Implementation of distributed parallelism}

This section describes the parallelization of the stages of the
algorithm's main loop. All but the first stage,
mini-batch selection (line 3 in Algorithm~\ref{algorithm}),
justify parallelization for
high values of combinations of the parameters mini-batch size $|\Minibatch|$,
number of vertices in the mini-batch $M$,
number of communities $K$, and sizes of neighbor sample set~$|\Neighbors|$ and held-out
set~$|\cE_h|$.

Mini-batch selection is done by the master and it is
...
$M \times K$ operations, and it is done in parallel over mini-batch vertices. For
memory consistency, this stage awaits completion of \textit{update\_phi} with
an MPI barrier. After the calculation, the updated values of $\pi$ and $\sum\phi$
...
is split into four steps. The first step partitions the mini-batch across
...
\subsubsection{Pipelining of computation and loading $\pi$}
