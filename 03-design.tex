...
high values of combinations of the parameters mini-batch size $|\cV_n|$,
number of communities $K$, neighbor sample size $|\cV_t|$, and held-out set
size $|\cE_h|$.
...
only in calculation but also in memory accesses. It performs $|\cV_n| \times |\cV_t| \times
...
$\pi/\phi$ for the mini-batch vertices. The update to $\pi$ requires $|\cV_n| \times
...
\textit{update\_beta} (lines 10 and 11 in Algorithm~1) requires $|\cV_n| \times K$ operations. It is preceded by an MPI
