...
Realizing this opportunity, however, requires significant efforts because
...
GPUs. The GPUs are highly effective but only when the problem state
fits in GPU memory. In order to address larger graphs and numbers of communities,
we created another implementation
that partitions the state across a cluster of machines. This allows processing
much larger problem instances at the cost of (memory) speed, because the
cluster nodes must read remote memory hosted by their peers. Since the
algorithm is data-bound, we substantially optimized remote memory accesses
by implementing a custom RDMA store.
...
\end{comment}
This implementation is the first to solve the largest publicly available
community graphs.
...
published at CCGrid-2016~\cite{10.1109/CCGrid.2016.98}. The technical
description and evaluation of our contributions
in Sections \ref{sec-design-sequential} and~\ref{sec-gpu},
Table~\ref{table-python-comparison}
and Figures \ref{gpu-sweep}, \ref{gpus-fig}
...
award. In summary, about 40\% of the work in this paper consists of new, unpublished
material.
