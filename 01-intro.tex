...
Python. Then, our key contributions to the algorithm are:

\begin{itemize}
\item \textbf{scaling up to run on GPUs.}
The application is not a natural fit for GPUs, because it is memory-bound and
...
\item \textbf{the use of pipelining} allows to use straightforward intra-node
thread parallelism in the distributed implementation, and it is pointless
to combine the GPU work into it. The cause is that the computation can be
